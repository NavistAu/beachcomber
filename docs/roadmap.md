...
- [ ] Consolidate high-volume low-risk test matrices — `tests/status_shape.rs` (~55 `#[test]`), `tests/config.rs` (~43), provider shape/metadata tests dominate count without exercising new lines. Convert to table-driven loops or snapshot-style with readable failure messages, then redirect freed effort to cold-spot modules. (Smell #3)

...
- [ ] Replace sleep-based waits with controllable clocks; share git-provider fixtures — `provider_git` dominates suite runtime; several scheduler/lifecycle tests use real-time sleeps. Inject test clocks into the scheduler and use fixture-cached git repos to keep individual tests cheap and non-flaky. Suite stays fast under parallelism today but individual slow tests are flake risk. (Smell #10)

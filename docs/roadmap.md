...
- [ ] Target the cold-spot files for direct coverage — priority: `provider/library.rs` (~20%), `provider/http.rs` (~29%), `proc_snapshot.rs` (~21%), `provider/kubecontext.rs` (~39%), `daemon.rs` (~47%), `singleton.rs` (~53%). Sequencing: do the boundary traits and lifecycle split first; coverage-by-construction is cheaper than coverage-after-the-fact. — Phase 3 status (2026-04-27): `proc_snapshot.rs` 96.02% ✓, `singleton/mod.rs` 76.30% ✓, `daemon/lifecycle.rs` 100% ✓. Still below 75%: `main.rs` 74.39% (0.61pp gap), `daemon/mod.rs` 55.43%, `provider/http.rs` 66.14%, `provider/library.rs` 57.98%, `provider/kubecontext.rs` 25.00%. The 0% boundary real-impl files (`boundaries/http.rs`, `boundaries/library.rs`, `boundaries/spawn.rs`) require live network/FFI/fork — deferred to cdylib fixture phase.
...
- [ ] Add `BEACHCOMBER_SOCKET` env var override for client socket path — the Phase 2 golden tests had to use `XDG_RUNTIME_DIR` override to redirect socket discovery; there is no `BEACHCOMBER_SOCKET` env var. This is a real product gap: users and integrations cannot easily point a client at a non-default socket without overriding `XDG_RUNTIME_DIR`. Implement `BEACHCOMBER_SOCKET` in socket path resolution (config override → `$BEACHCOMBER_SOCKET` → `$XDG_RUNTIME_DIR/beachcomber/sock` → `$TMPDIR/beachcomber-<uid>/sock`); expose it in the Rust client `ClientConfig::from_env`; document it in `config.rs` and the website reference.

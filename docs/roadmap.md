...
- [x] Add a coverage gate to CI with slow ratchet — shipped `ci/coverage-gate.sh` at `--fail-under-lines 66` baseline (measured 66.14% on 2026-04-27). Wired into CI as a dedicated `coverage` job on `macos-latest`. **Coverage gate ratchet policy:** The gate baseline lives in `ci/coverage-gate.sh` (`COVERAGE_BASELINE`). It is raised only after a phase lands that meaningfully improves coverage, and only by an amount that does not require ad-hoc test additions to clear. Never lower it without an explicit rationale committed alongside.

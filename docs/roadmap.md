...
- [ ] Define boundary traits for side-effecting code so the operational shell becomes testable — process execution, socket discovery, daemon spawn, filesystem/env access, HTTP client, dynamic library loader. Currently these are inlined into `daemon.rs`, `singleton.rs`, `proc_snapshot.rs`, `provider/http.rs`, `provider/library.rs`, which is exactly why those files sit at 20–53% while pure-logic neighbours (cache, scheduler, formatting) sit at 79–92%. (Smell #2)
...
- [ ] Split process-lifecycle policy from OS effects in `daemon.rs`, `singleton.rs`, `proc_snapshot.rs` — pid resolution, stale pidfile handling, binary-hash freshness, self-watch, orphan reaping, lock contention, kill-with-timeout. Pure decision logic gets unit tests; OS calls get a small ring of integration tests. Replaces today's coarse "spawn and observe" coverage. (Smell #8)
- [ ] Convert operational integration tests to deterministic units + fewer smoke tests — daemon protocol/client suite is 87 tests / 13.8s; many edge cases (stale sockets, fork errors, malformed library metadata, timeout paths) are hard to trigger via integration. Targets unit-testable seams created by the boundary-trait work above. (Smell #5)
- [ ] Target the cold-spot files for direct coverage — priority: `provider/library.rs` (~20%), `provider/http.rs` (~29%), `proc_snapshot.rs` (~21%), `provider/kubecontext.rs` (~39%), `daemon.rs` (~47%), `singleton.rs` (~53%). Sequencing: do the boundary traits and lifecycle split first; coverage-by-construction is cheaper than coverage-after-the-fact.

...
- The beachcomber daemon must be running. Verify with `comb status` from a terminal.
...
If you do not want to install the Lua SDK, you can call the `comb` binary directly using `vim.fn.system()`. This forks a process on each call, but because `comb get` returns in under 1ms it is still fast enough for statusline use:
...
local function comb_get(key, path)
    local cmd = path
        and string.format('comb get %s %s -f text', key, vim.fn.shellescape(path))
        or  string.format('comb get %s -f text', key)
...
- **Without the SDK:** the `vim.fn.system()` approach has no dependency to troubleshoot — if `comb get git.branch . -f text` works in your terminal, it works in neovim.
